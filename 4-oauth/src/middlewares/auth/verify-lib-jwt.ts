...
import { verifyLibJWT } from '../../utils/jwt/verify';

const verifyTokenWithJWT = async (req: Request, res: Response, next: NextFunction) => {
  const {
    headers,
  } = req;
  ...
  try {
    ...
    const user: UserSchemaType = verifyLibJWT(accessToken);
